...
// Patch 3: cap awaitRequestsFinished at 2000 ms
...
// 2 s gives image/API requests time to settle (they normally respond in <1 s)
// while still cutting off the persistent connections that never resolve.
patch(
    path.join(root, '@loki', 'target-chrome-core', 'src', 'create-chrome-target.js'),
    `      debug('Waiting for awaitRequestsFinished...');
      await awaitRequestsFinished();`,
    `      debug('Waiting for awaitRequestsFinished...');
      await Promise.race([awaitRequestsFinished().catch(() => {}), new Promise(r => setTimeout(r, 2000))]);`,
    'create-chrome-target: cap awaitRequestsFinished at 2000 ms',
)
