...
({ className, articleId }: ArticleRatingProps): JSX.Element {
  ...
  const rateArticleHandle = useCallback((starsCount: number, feedback?: string): void => {
    try {
      void rateArticle({
        userId,
        articleId,
  ...
  const onAcceptHandle = useCallback((starsCount: number, feedback?: string): void => {
    rateArticleHandle(starsCount, feedback)
  }, [rateArticleHandle])

  const onCancelHandle = useCallback((starsCount: number): void => {
